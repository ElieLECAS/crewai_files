...
function initUploadSystem() {
    ...
    uploadForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (data.success) {
                showStatus(`✅ ${data.message}. Suivez la progression ci-dessous.`, 'success');
                fileInput.value = '';
                updateFileList([]);
    ...
    function updateTasksUI(tasks) {
        if (tasks.length === 0) {
            tasksList.innerHTML = '<div class="empty-state"><p>Aucun traitement récent</p></div>';
            return;
        }
        ...
        tasksList.innerHTML = tasks.map(task => {
            ...
            switch(task.status) {
                case 'completed':
                    icon = '<svg class="success" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>';
                    break;
                case 'failed':
                    icon = '<svg class="error" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>';
                    break;
                case 'processing':
                    icon = '<svg class="primary spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>';
                    break;
                default:
                    icon = '<svg class="secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';
            }
    ...
    async function refreshStats() {
        try {
            ...
            for (const [name, data] of Object.entries(stats)) {
                const card = document.querySelector(`.stat-card h3:contains("${data.display_name}")`);
                if (card) {
                    const numberEl = card.parentElement.querySelector('.stat-number');
            ...
            document.querySelectorAll('.stat-card').forEach(card => {
                ...
                for (const [name, data] of Object.entries(stats)) {
                    if (title === data.display_name.toLowerCase()) {
                        card.querySelector('.stat-number').textContent = data.count;
                    }
                }
            });
            
            showStatus('✅ Statistiques mises à jour automatiquement', 'success');
        } catch (error) {
